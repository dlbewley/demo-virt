...
export TOKEN=$(oc whoami -t) # or use OpenShift web console to obtain a token
----
...
Browse to Migration -> Providers for virtualization and create provider by hand or start with 
...
Creating a VMware provider will require the SHA1 fingerprint of the vCenter server.

* Obtain the SHA-1 fingerprint for the vCenter certificate and write this value to link:base/secrets/thumbprint[base/secrets/thumbprint]
[source,bash]
...
* https://access.redhat.com/documentation/en-us/migration_toolkit_for_virtualization/2.4/html/installing_and_using_the_migration_toolkit_for_virtualization/migrating-vms-web-console#adding-providers
[Create a example VMware provider]
...
* Edit the provider and correct the credentials and certificate SHA-1 fingerprint.
...
It might be good to create a migration-in namespace with a NAD there.
