...
Virtual machines can use the default pod network and expose ports using Services, Routes etc, but it is likely you may have a need to deploy a virtual machine directly to existing networks outside the cluster.
...
Some generated NNCPs can be found here link:networking[demos/components/networks/].
